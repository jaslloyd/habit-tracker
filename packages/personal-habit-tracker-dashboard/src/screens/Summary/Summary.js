...
class Summary extends Component {
  ...
  async componentDidMount() {
    const habits = await (await fetch(`${process.env.REACT_APP_API_ENDPOINT}/api/occurrence_habits`)).json();
  ...
  render() {
    ...
    const tableElements = this.state.habitsTable.map(key => (
      <SummaryTable key={key} name={key} values={this.state.habitsTable[key]} />
    ));
